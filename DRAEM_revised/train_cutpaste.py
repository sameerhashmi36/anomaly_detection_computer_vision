...
def train_on_device(obj_names, args):
    ...
    for obj_name in obj_names:
        ...
        run_name = f'DRAEM_test_{args.lr}_{args.epochs}_bs{args.bs}_{obj_name}_'
        ...
        dataset = MvtecCutPaste(
            args.data_path + obj_name + "/train/good/",
            args.anomaly_source_path + obj_name + "/train/good/",
        ...
        for epoch in range(args.epochs):
            ...
            for i_batch, sample_batched in progress_bar:
                ...
                print("Gray Rec Min:", gray_rec.min().item(), "Max:", gray_rec.max().item(), "Mean:", gray_rec.mean().item())
                print("Gray Batch Min:", gray_batch.min().item(), "Max:", gray_batch.max().item(), "Mean:", gray_batch.mean().item())


                pixelwise_loss = (gray_rec - gray_batch).pow(2)
                print("Max Pixel-wise L2 Loss:", pixelwise_loss.max().item())


                # Compute losses
                l2_loss = loss_l2(gray_rec, gray_batch)
                # l2_loss = loss_l2(gray_rec.float()/255.,gray_batch.float()/255.)
                # l2_loss = loss_l2(gray_rec, gray_batch/255.0)
